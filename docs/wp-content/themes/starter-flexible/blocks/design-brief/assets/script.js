...
document.querySelectorAll('[data-design-brief]').forEach((form) => {
	...
	// Wording for what the script writes itself; PHP owns the translations.
	const T = letterhead.strings ?? {};
	const isEn = Boolean(letterhead.is_en);
	...
	const initEngine = (engine) => {
		...
		const fillModels = () => {
			...
			if (!rows.length) {
				...
				empty.textContent = make ? T.noModels ?? '' : T.pickMake ?? '';
		...
		[...form.querySelectorAll('.db__section')]
			.map((section) => {
				...
				const blocks = engines.length
					? engines.map((engine, i) => ({ title: `${T.engine ?? 'Máy'} ${i + 1}`, groups: groupsOf(engine) }))
	...
	const documentHtml = () => {
		...
		const grounds = [
			`${T.basis1} ${esc(number)}${requestDate ? ` ${T.dated} ${esc(requestDate)}` : ''}${customer ? ` ${T.by} ${esc(customer)}` : ''};`,
			T.basis2,
			T.basis3,
...
<title>${esc(T.docTitle)}${code ? ' ' + esc(code) : ''}</title>
<style>
...
<table class="head"><tr>
	<td class="left">
		...
		<div class="rule"></div>
		<p class="no">${esc(T.docNumber)}: ${esc(number)}/${esc(symbol)}</p>
	</td>
	<td class="right">
		${isEn ? '' : `<p class="nation">Cộng hòa xã hội chủ nghĩa Việt Nam</p>
		<p class="motto">Độc lập - Tự do - Hạnh phúc</p>
		<div class="rule rule--wide"></div>`}
		<p class="place">${place ? esc(place) + ', ' : ''}${isEn ? today.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' }) : longDate(today)}</p>
	</td>
</tr></table>

<h1>${esc(T.docTitle)}</h1>
<p class="subject">${[code && `${T.designRef}: ${esc(code)}`, vessel && `${T.registration}: ${esc(vessel)}`].filter(Boolean).join(' — ') || '&nbsp;'}</p>
...
<table class="sign"><tr>
	<td>
		<p class="role">${esc(T.client)}</p>
		<p class="hint">${esc(T.signHint)}</p>
		<div class="space"></div>
		<p>${esc(customer)}</p>
	</td>
	<td>
		<p class="role">${esc(T.designer)}</p>
		<p class="hint">${esc(T.signHintSeal)}</p>
		<div class="space"></div>
...
<div class="recipients">
	<p class="title">${esc(T.recipients)}</p>
	<p>${T.recipientList}</p>
