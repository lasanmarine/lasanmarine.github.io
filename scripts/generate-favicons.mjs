...
const W = 372.097, H = 138.012;
const pad = 0.08; // 8% padding each side
const side = W / (1 - 2 * pad);
const x = side * pad - 0; // left offset for content
const y = (side - H) / 2;

const square = (bg) => `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${side}" height="${side}" viewBox="0 0 ${side} ${side}">
${bg ? `<rect width="${side}" height="${side}" fill="${bg}"/>` : ''}
<g transform="translate(${x}, ${y})">${inner}</g>
</svg>`;

await fs.writeFile('/private/tmp/claude-501/-Users-nguyenquangvinh-Desktop-LasanMarine-lasanmarine-github-io/94e104ce-e73a-4bf3-8bbc-271500be4ed6/scratchpad/square.svg', square(null));

const render = (svg, size) =>
  sharp(Buffer.from(svg), { density: 600 }).resize(size, size, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } }).png({ compressionLevel: 9 }).toBuffer();
